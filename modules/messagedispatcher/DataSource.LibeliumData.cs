namespace messagedispatcher
{
    ...
    public class LibeliumData
    {
        ...
        public LibeliumData(string timestamp, string croptype, string topic, string payload)
        {
            Timestamp = timestamp;
            CropType = croptype;
            Topic = topic;
        ...
        public NormalizedData ToNormalizedData()
        {
            ...
            float soil = float.Parse(data["soilmoisture"]);
            float co2 = float.Parse(data["co2"]);
            float soilph = float.Parse(data["soilph"]);
            float solarradiation =  float.Parse(data["solarradiation"]);
            ...
            return new NormalizedData(Timestamp, Topic, deviceid, CropType, temp, hum, light, soil, co2, soilph, solarradiation);
        }
    }
